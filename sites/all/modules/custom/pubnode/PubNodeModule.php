...
//    parent::__construct("pubnode", "Publication Node");
  ...
  public function load ($node) {
    $result = db_query("SELECT pgid, idx, display_title, global_id, doctype, docpath, pubpath, pdfpath, toc, pubdate FROM {pubnode} WHERE vid = :vid", array(':vid' => $node->vid));
...
//     return $pn;
  ...
  // imported from oo, returns the name of the module / node type.
...
//       );
      ...
      $form['authors'] = array(
        '#title' => t('Authors'),
        '#markup' => theme('pubnode_authors', $authors),
        '#weight' => $wt++
      );
...
//       );
      ...
      $form['teaser'] = array(
        ...
        '#title' => t('Teaser'),
        '#rows' => 8,
        '#default_value' => isset($node->body[LANGUAGE_NONE][0]['summary']) ? $node->body[LANGUAGE_NONE][0]['summary'] : '',
        '#weight' => $wt++
      );
...
//       );
      ...
      $form['body'] = array(
        '#type' => 'value',
        '#value' => isset($node->body[LANGUAGE_NONE][0]['safe_value']) ? $node->body[LANGUAGE_NONE][0]['safe_value'] : '',
        );
      ...
      $form['toc_preview']['tp'] = array(
        '#markup' => isset($node->toc) ? $node->toc : ''
      );
      ...
      $form['body_preview']['bp'] = array(
        '#markup' => isset($node->body[LANGUAGE_NONE][0]['safe_value']) ? $node->body[LANGUAGE_NONE][0]['safe_value'] : ''
      );
